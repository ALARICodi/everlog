...
function run() {
  ...
  for (const c of btc) {
    ...
    html += '<p>上面那串数字,应当正好是<b>比特币第 ' + c.attestation.height + ' 号区块的 Merkle Root</b>。</p>'
    html += '<p class="note">打开下面任意一个链接,找到页面上的 <b>Merkle Root</b> 一栏,和上面那串绿色的字逐字比对。' +
            '一致,就说明你手里这份文字在那个区块被挖出之前就已经存在了。</p>'
