 ...
 * 分配之后锁死:没有任何接口提供改号途径。
 */
export async function createAccount(passwordHash, faceEnrolled, realName) {
  const name = normalizeName(realName)
  if (!name) throw new Error('缺少真实姓名')
  ...
  const user = {
    ...
    status: 'active',             // 号、人脸、姓名一次到位,建号即认证完成
    faceEnrolled: !!faceEnrolled,
    // 姓名存四份,各司其职:
    realNameHash: hashPassword(name),   // 验证(带随机盐,不可查)
    realNameIndex: nameIndex(name),     // 查找(确定性 HMAC,支持姓名登录)
    realNameEnc: encryptName(name),     // 披露(分享密钥持有者可见全名)
    realNameMasked: maskName(name),     // 显示(陈**)
  }
...
/** 全部用户(只在按姓名登录时用,量大了要换索引文件) */
async function allUsers() {
  let names = []
  try { names = await fs.readdir(USERS) } catch { return [] }
  const out = []
  for (const n of names) {
    if (!n.endsWith('.json')) continue
    try { out.push(JSON.parse(await fs.readFile(path.join(USERS, n), 'utf8'))) } catch {}
  }
  return out
}
...
/**
 * 姓名的**确定性索引**,用来支持「姓名 + 密码」登录。
 *
 * 为什么不能直接拿 scrypt 哈希去查:那个哈希每人一个随机盐,
 * 同一个名字算出来的值各不相同 —— 能验证,但没法查找。
 *
 * 所以另存一份 HMAC(姓名, 服务端密钥)。同一个名字永远得到同一个值,可以拿来查;
 * 而没有密钥就反推不出姓名,泄露了也读不出人名。
 *
 * **注意:同名的人会撞到同一个索引** —— 这不是 bug,是现实。
 * 所以按姓名登录时会拿到一组候选,再用密码区分;
 * 万一密码也一样(极小概率),就要求改用用户号登录。
 */
function nameIndex(normalized) {
  return crypto.createHmac('sha256', nameKey()).update(normalized).digest('hex')
}

const FAIL_MSG = '账号或密码不对'
 ...
 *   真实姓名 + 密码
 *
 * 失败一律给同一句话,不透露账号存不存在、也不透露是哪个因子错的。
 *
 * @param {string} account 用户号(U + 8 位数字)或真实姓名,自动分辨
 */
export async function login(account, password) {
  const acct = String(account || '').trim()
  const pw = String(password || '')

  if (isValidUserId(acct.toUpperCase())) {
    const u = await read(acct.toUpperCase())
    if (!u) {
      // 号不存在也走一遍 scrypt,让耗时看起来一样,别让人靠响应快慢探号
      crypto.scryptSync(pw, Buffer.alloc(16), 64, { N: 16384, r: 8, p: 1 })
      throw new Error(FAIL_MSG)
    }
    if (!verifyPassword(pw, u.password)) throw new Error(FAIL_MSG)
    return publicView(u)
  }

  // 按姓名:先用确定性索引捞出所有同名的人,再用密码从中定位
  const name = normalizeName(acct)
  if (!name) throw new Error(FAIL_MSG)
  const idx = nameIndex(name)
  const candidates = (await allUsers()).filter(u => u.realNameIndex === idx)
  ...
  const matched = candidates.filter(u => verifyPassword(pw, u.password))
  if (matched.length === 1) return publicView(matched[0])
  if (matched.length === 0) throw new Error(FAIL_MSG)

  // 同名 + 同密码。概率极小,但真撞上就不能瞎猜给谁登录。
  throw new Error('有多个同名账号使用了相同密码,请改用用户号登录')
}
